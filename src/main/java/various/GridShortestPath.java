 ...
 * several shortcuts are taken to minimize the code.  NOT a demonstration of
 * bulletproof code and not best practices, e.g equals() assumes it will not be
 * passed null or a type other than Position, and no attempt to encapsulate 
 * object state. 
 */
public class GridShortestPath {
	 ...
	 *
	 * @param start - the initial Position in the grid 
	 * @param goal - the end Position in the grid
	 * @return a shortest path solution
	 */
	...
	getShortestPath(Position start, Position goal) {
		...
		if (path.size() > 1) 
			path.get(0).pathweight = path.get(1).pathweight + 1;
		this.start.pathweight = 0;
		path.add(this.start);
	...
	/*
	 * unfortunately, this is a O(n * m) algorithm
	 */
	public String showPath(List<Position> path) {
		StringBuilder sb = new StringBuilder();
		for (int i = 0; i < n; i++)
			for (int j = 0; j < m; j++) {
				Position p = new Position(i, j, inf);
				int index = path.indexOf(p);
				if (index == -1) sb.append("*");
				else sb.append(path.get(index).pathweight);
				if (j == m - 1) sb.append('\n');
			}
	...
	public static void main(String[] args) {
		...
		List<Position> path = 
				g.getShortestPath(g.new Position(4,2,g.inf), 
						g.new Position(6,76,g.inf));
